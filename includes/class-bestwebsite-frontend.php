...
class bestwebsite_Frontend {
    ...
    public function includes(){
        ...
        include_once 'bestwebsite-meta-tag-list.php';
        include_once 'class-bestwebsite-retrieve-info.php';
        include_once 'class-bestwebsite-retrieve-tags.php';

        $this->meta_tag_list = $bestwebsite_meta_tag_list;
    ...
    public function set_html_prefix_attribute(){
        ...
        add_filter( 'language_attributes', 'bestwebsite_add_og_html_prefix' );

        function bestwebsite_add_og_html_prefix( $output ){
    ...
    public function process_auto_tags( $tag_to_process ){
        ...
        if ( empty($this->page_info) ){
            $this->page_info = new bestwebsite_Retrieve_Info( $this->page_type, $this->page_id );
        }



        // general tags - we skip keywords because it's not recommended
        if( $tag_to_process == 'bestwebsite_general_description' ){
        ...
        // open graph
        if( $tag_to_process == 'bestwebsite_og_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'bestwebsite_og_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'bestwebsite_og_type' ){
        ...
        if( $tag_to_process == 'bestwebsite_og_audio' ){
            return $this->page_info->audio;
        }

        if( $tag_to_process == 'bestwebsite_og_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'bestwebsite_og_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'bestwebsite_og_video' ){
            return $this->page_info->video;
        }

        if( $tag_to_process == 'bestwebsite_og_url' ){
        ...
        // twitter cards
        if( $tag_to_process == 'bestwebsite_twitter_card' ){

            if (
                ! empty( $this->page_info->video ) &&
                ( ! empty( $this->tags['bestwebsite_twitter_player'] ) || ! empty( $this->tags['bestwebsite_twitter_player_stream'] ) )
            ){
                return 'player';
            }
        ...
        if( $tag_to_process == 'bestwebsite_twitter_title' ){
            return $this->page_info->title;
        }

        if( $tag_to_process == 'bestwebsite_twitter_description' ){
            return $this->page_info->description;
        }

        if( $tag_to_process == 'bestwebsite_twitter_image' ){
            return $this->page_info->image;
        }

        if( $tag_to_process == 'bestwebsite_twitter_image_alt' ){
            return $this->page_info->image_alt;
        }

        if( $tag_to_process == 'bestwebsite_twitter_player_stream' ){
        ...
        if( $tag_to_process == 'bestwebsite_twitter_player_stream_content_type' ){
    ...
    public function process_tags(){
        ...
        $taginfo = new bestwebsite_Retrieve_Tags( $this->meta_tag_list );
        ...
        $bestwebsite_autopilot_settings = get_option('bestwebsite_autopilot_settings');
        $autopilot_arr = maybe_unserialize($bestwebsite_autopilot_settings);
        ...
        foreach ( $this->meta_tag_list as $group => $values ){
            ...
            foreach ($values['fields'] as $field => $info) {
                ...
                if (
                    ...
                    if ( $info['variable'] == 'bestwebsite_custom' ){
                    // user defined extra meta tags
                        ...
                        if ( ! $this->is_og_html_attr_set && substr($info['variable'], 0, 7) == 'bestwebsite_og' ){
...
return new bestwebsite_Frontend();
