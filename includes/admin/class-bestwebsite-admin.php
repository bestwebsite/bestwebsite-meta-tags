...
class bestwebsite_Admin {    
    ...
    public function __construct(){
        ...
        register_activation_hook( bestwebsite_PLUGIN_FILE, array( $this, 'on_activation' ) );
        ...
        add_filter( 'plugin_action_links_' . bestwebsite_PLUGIN_FILE, array( $this, 'add_action_link' ) );
        ...
        add_action( 'admin_post_bestwebsite_editor_form_submit', array( $this, 'save_meta_tags' ) );
        add_action( 'admin_post_bestwebsite_table_bulk_submit', array( $this, 'table_bulk_actions' ) );
    ...
    public function on_update( $upgrader_object, $options ){
        ...
        if( $options['action'] == 'update' && 
            ...
            if ( in_array( bestwebsite_PLUGIN_FILE, $options['plugins'] ) ){
    ...
    public function load_textdomain(){
        ...
        load_plugin_textdomain( 'dp-meta-tags', false, bestwebsite_PLUGIN_DIR .'/languages' );
    ...
    public function check_version(){
        ...
        // run specific tasks
        if ( empty( get_option( 'bestwebsite_plugin_version' ) ) ){
                       
            include_once 'class-bestwebsite-migration.php';
            $migrate = new bestwebsite_Migration();
        ...
        $plugin_info = get_plugin_data( bestwebsite_PLUGIN_FULL_PATH );
        if ( $plugin_info['Version'] != get_option( 'bestwebsite_plugin_version' )  ){
            
            update_option( 'bestwebsite_plugin_version', $plugin_info['Version'] );
    ...
    public function meta_tag_pages(){
        ...
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/bestwebsite-meta-tag-list.php';
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/class-bestwebsite-retrieve-tags.php';

        if ( ! empty($_GET['type']) && ! empty($_GET['edit']) ){

            // editor page            
            include_once dirname( plugin_dir_path( __FILE__ ) ) . '/class-bestwebsite-retrieve-info.php';
            include_once 'views/html-meta-tag-editor.php';        

        }else{

            // table page
            include_once 'class-bestwebsite-retrieve-list.php';
    ...
    public function set_notices(){
        ...
        add_action( 'admin_notices', function() {
            ...
            if( $screen == 'themes.php' && ! get_user_meta( $user_id, 'bestwebsite_ad_dismissed' ) ){
                ...
                echo '<span class="dpmt-dismiss-forever"><a href="?bestwebsite_ad_dismissed=1"><i class="dashicons dashicons-dismiss"></i> ';
        ...
        // dismiss notice
        if ( isset($_GET['bestwebsite_ad_dismissed']) ){
            
            $user_id = get_current_user_id();
            add_user_meta( $user_id, 'bestwebsite_ad_dismissed', 'true', true );
    ...
    public function add_css_js(){
        ...
        wp_enqueue_style( 'bestwebsite_admin_css', plugins_url('assets/css/admin.css', bestwebsite_PLUGIN_FILE) );
        wp_enqueue_script( 'bestwebsite_admin_js', plugins_url('assets/js/admin.js', bestwebsite_PLUGIN_FILE), array('jquery') );
    ...
    public function save_meta_tags(){
        ...
        include_once dirname( plugin_dir_path( __FILE__ ) ) . '/bestwebsite-meta-tag-list.php';
        include_once 'class-bestwebsite-save-tags.php';       

        bestwebsite_Save_Tags::save( $bestwebsite_meta_tag_list, $_POST );


        // redirect to previous page
        wp_redirect( admin_url( 'options-general.php?page=dpmt-editor&type='. $_POST['bestwebsite_type'] .'&edit='. $_POST['bestwebsite_id'] ) );
    ...
    public function table_bulk_actions(){
        ...
        foreach( $groups as $group ){
            ...
            if ( $_POST['bulk-'. $group] != -1 ){
                ...
                bestwebsite_Save_Tags::bulk( $_POST['bulk-'. $group], $bestwebsite_meta_tag_list, $type, $group );
    ...
    public function add_meta_boxes(){
        ...
        foreach ($screens as $screen) {
            add_meta_box(
                'bestwebsite_meta_tag_editor_link',
    ...
    public function always_autopilot_setting(){
        ...
        $array_to_save = maybe_unserialize( get_option('bestwebsite_autopilot_settings') );
        ...
        update_option('bestwebsite_autopilot_settings', $array_to_save);
