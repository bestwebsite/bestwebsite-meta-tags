...
class bestwebsite_Save_Tags {
    ...
    public static function save( $list_of_meta_tags, $data ){
        ...
        // validate type
        if ( ! post_type_exists($data['bestwebsite_type']) && ! taxonomy_exists($data['bestwebsite_type']) ){
            return;
        }

        $type = $data['bestwebsite_id'] == 'front' ? 'frontpage' : $data['bestwebsite_type'];

        $url_fields = [ 
            'bestwebsite_og_audio',
            'bestwebsite_og_image',
            'bestwebsite_og_video',
            'bestwebsite_og_url',
            'bestwebsite_twitter_image',
            'bestwebsite_twitter_player_stream'
        ];
            ...
            foreach ( $item['fields'] as $tag => $field ){
                ...
                switch (true){
                    ...
                    case taxonomy_exists( $type ):
                        update_term_meta( $data['bestwebsite_id'], $key, $value );

                        break;


                    case $type == 'author':
                        update_user_meta( $data['bestwebsite_id'], $key, $value );

                        break;


                    case $type == 'frontpage':
                        update_option( 'bestwebsite_frontpage_'. $key, $value );

                        break;


                    case post_type_exists( $type ):
                        update_post_meta( $data['bestwebsite_id'], $key, $value );
    ...
    public static function bulk( $action, $list_of_meta_tags, $wp_object_type, $meta_tag_group ){
        ...
        $items = bestwebsite_Retrieve_List::get_list( $wp_object_type );
                ...
                if ( $item['id'] == 'front' ){
                    ...
                    foreach ( $fields_to_update as $field ){
                        ...
                        if ( $action == 'autopilot' ){
                            ...
                            update_option( 'bestwebsite_frontpage_'. $field, 'auto' );

                        }elseif ( $action == 'delete' ){
            
                            delete_option( 'bestwebsite_frontpage_'. $field );                            
